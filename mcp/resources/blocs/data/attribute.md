# HTMLタグの特定の属性値を置き換える方法
テンプレート内でHTMLタグの属性値を変数で置き換えるには、`data-attribute` 属性を使います。置き換えたい属性名を `data-attribute` に指定し、代入する値を `data-val` に記述します。もし指定した属性がタグに存在しない場合は、その属性が新しく追加されます。この方法は、タグ記法でもコメント記法でも使用できます。
