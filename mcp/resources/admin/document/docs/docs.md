# 処理機能記述書を自動生成するミドルウェア docs の使い方
`docs` は、Laravelのコントローラーに処理内容をコメントとして記述することで、Excel形式の処理機能記述書を自動生成できるツールです。開発におけるドキュメント作成の負担を軽減することを目的としており、コントローラーに記述した処理説明をもとに、画面表示などの実行時にミドルウェアがその内容を読み取り、Excelドキュメントを自動的に生成します。これにより、手作業によるドキュメント作成が不要となり、開発効率の向上が期待できます。

## ドキュメント生成手順
このツールでは、Laravelのコントローラーに処理説明を記述し、対象ルートにアクセスするだけで、Excel形式の処理機能記述書を自動生成できます。以下の手順で設定・利用します。

### 1. コントローラーに処理説明を書く
**docs()** 関数を使って、処理内容を順番に記述します。記述した順に、ドキュメントに反映されます。
```php
